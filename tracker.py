...
def pick(result, hotel):
    """Return (name, nightly, total, source). Handles a list of properties or a single-property page."""
    if result.get("error"):
    ...
    if props:
        ...
        return found["name"], nightly, total, "Google Hotels"
    ...
    t, n, s = min(rows)
    return result.get("name", hotel["name"]), n, t, s
...
def main():
    ...
    data.update(
        ...
        hotels=[{"id": h["id"], "name": h["name"], "sv": h.get("sv", "")} for h in CFG["hotels"]],
    )
    ...
    for h in CFG["hotels"]:
        try:
            name, nightly, total, source = pick(search(h["query"]), h)
        except Exception as e:
        ...
        hist.append(
            {"t": now, "hotel": h["id"], "total": round(total), "nightly": round(nightly), "source": source}
        )
